...
function mapStatus(status: number): SpotifyError {
  ...
  if (status === 403)
    return { code: "premium_required", message: "Premium required" };
  if (status === 404)
    return { code: "no_active_device", message: "No active device" };
...
export async function getPlayer(
  ...
  try {
    ...
    if (res.status === 204)
      return { ok: true, value: { isPlaying: false, track: null } };
